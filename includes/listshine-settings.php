 ...
 */
  ...
  $url = http_build_query( array(
          'next'          =>  admin_url('admin.php?page=listshine-settings'),
          'scope'         =>  '',
          'response_type' =>  'json'
      )
  );
  ...
  if ( isset( $_POST["save_code"]) and isset($_POST["access_code"]) ) {
      ...
      if( $wp_listshine->wpa_save_data( $_POST["access_code"] )){
...
<div class="wrap">
  <span class='opt-title'><span id='icon-options-general' class='analytics-options'><img src="<?php echo plugins_url('wp-analytify/images/wp-analytics-logo.png');?>" alt=""></span>
      ...
      if ( isset ( $_GET['tab'] ) ) $wp_listshine-> pb_settings_tabs($_GET['tab']);
      else $wp_listshine->pa_settings_tabs( 'authentication' );
    ...
    <table width="1004" class="form-table">
      ...
      if( $tab == 'profile' ){
          $profiles = $wp_listshine->pt_get_analytics_accounts();
          if( isset( $profiles ) ) { ?>
